...
local animScale = 1
local bulletScale = 2 * deviceScaleFactor
-- Parâmetros do ataque (ajuste conforme necessário)
local baseDamageMultiplier = 0.55

-- Controle de escala para a animação de hit (já considerando o global)
local hitAnimScale = 1 * deviceScaleFactor -- ajuste conforme necessário
...
local hitSequenceData = {
    ...
    start = 1,
    count = 6,
    time = 300, -- tempo da animação de hit
    loopCount = 1
}
...
local upSheet = graphics.newImageSheet("assets/7effect/action_109.png", upSheetOptions)
...
local extra2Sheet = graphics.newImageSheet("assets/7effect/action_109.png", extra2SheetOptions)
...
local function playUpAnimation(cardGroup, callback)
    -- 1) Cria o sprite e insere no cardGroup
    local upSprite = display.newSprite(upSheet, upSequenceData)
    upSprite.anchorX, upSprite.anchorY = 0.5, 0.5
    upSprite.x, upSprite.y = 0, 0
    cardGroup:insert(upSprite)
    upSprite:toBack()

    -- 2) Inicializa alpha e escala
    upSprite.alpha = 0
    upSprite.xScale = 0
    upSprite.yScale = 0

    -- 3) Fade in + scale from 0 → 2.0 em 200ms
    ...
    -- 4) Prepara o fade out para começar 10ms antes de 500ms
    transition.to(upSprite, {
        delay = 500 - 10,
        time = 200,
        alpha = 0
        -- opcional: você pode também reescalar de volta a zero se quiser
        -- xScale    = 0,
        -- yScale    = 0,
    })

    -- 5) Quando a última frame do sprite animado terminar, remove e chama callback
    upSprite:play()
...
function rasengan.attack(attacker, target, battleFunctions, targetSlot, callback)
    ...
    _G.manageCardTurn(target, "applyBleed", bleedParams)
