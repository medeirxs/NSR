...
function naraka.attack(attacker, dummyTarget, battleFunctions, targetSlot, callback)
  ...
  transition.to(cardGroup, {
      ...
      onComplete = function()
          ...
          transition.to(overlayRect, { alpha = 0, time = overlayFadeTime, delay = overlayTotalTime - overlayFadeTime })

          -- 4) Ação contínua
          local actionSprite = createActionSprite(cardGroup)

          -- 5) Overlay animado no centro da tela
          showOverlayAnimationAtPosition(
              display.contentCenterX,
              display.contentCenterY,
              function()
                  ...
                  showHitAtPosition(center.x, center.y, function()
                      ...
                      if actionSprite and actionSprite.removeSelf then actionSprite:removeSelf() end
                      if overlayRect and overlayRect.removeSelf then overlayRect:removeSelf() end
