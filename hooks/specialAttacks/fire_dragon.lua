-- hooks/basicAttacks/move_and_sweep.lua
...
local function playActionAnimation(cardGroup, onComplete)
    ...
    local seqData     = { name="action", start=1, count=4, time=400, loopCount=1 }
    local sprite      = display.newSprite(actionSheet, seqData)

    sprite.anchorX, sprite.anchorY = 0.5, 0.5
    sprite.x, sprite.y             = 0, 0
    sprite:scale(deviceScaleFactor, deviceScaleFactor)
...
local function playActionAnimationAbove(cardGroup)
    ...
    local seqData     = { name="actionAbove", start=1, count=4, time=400, loopCount=1 }
    local sprite      = display.newSprite(actionSheet, seqData)
...
function move_and_sweep.attack(attacker, dummyTarget, battleFunctions, targetSlot, callback)
