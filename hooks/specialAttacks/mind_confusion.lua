...
local hitSheet = graphics.newImageSheet("assets/7effect/bullet_55.png", hitSheetOptions)
local hitSequenceData = {
    name = "hitAnim",
    start = 1,
    count = 9,
    time = 800, -- tempo da animação de hit
    loopCount = 1
}
                ...
                transition.to(cardGroup, {
                    ...
                    onComplete = function()
                        ...
                        timer.performWithDelay(hitSequenceData.time, function()
                            ...
                            if targetData then
                                _G.manageCardTurn(targetData, "block", 1)
                            end
                            if callback then
                                callback()
                            end
