...
local function simulateTurnActions()
	-- Corrige o bug do enemyFormation nil usando função de acesso tardio
	local function getEnemyFormation(attacker)
		return attacker.isOpponent and _G.playerFormationData or _G.opponentFormationData
	end

	local function getFormationCenter(attacker)
	...
	local function performAction()
		...
		-- Simplificação: removidas todas as fases, mantém apenas a única batalha
		if currentAction > totalActions then
			-- Se não há mais oponentes, encerra a batalha
			if countTeam(opponentFormationData) == 0 then
				print("Batalha encerrada. Jogador venceu!")
				return
			end

			-- Senão, refaz o loop de ações
...
function scene:create(event)
	...
	local enemieId = params.enemieId or 439310972 -- ID do oponente remoto (substitua pelo real)
	...
	-- Define o background
	local bgImagePath = "assets/7bg/campaign/" .. 7 .. ".jpg"
	local bgWidth = 1068
	local bgHeight = 2548
	...
	-- Carrega formação do jogador
	formationHook.fetchFormationData(userId, function(data, err)
		if err then
			native.showAlert("Erro", "Não foi possível carregar sua formação", { "OK" })
			return
		end
		...
		end, false) -- false: linha normal
	end)

	-- Carrega formação do inimigo (com enemieId)
	formationHook.fetchFormationData(enemieId, function(data, err)
		if err then
			native.showAlert("Erro", "Não foi possível carregar a formação do inimigo", { "OK" })
			return
		end

		opponentFormationData = data
