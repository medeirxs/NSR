        ...
        if event.isError then
            ...
            for i = 1, #newFormation do
                if type(newFormation[i]) == "table" then
                    ...
                    print("Carta: " .. (newFormation[i].name or "Sem Nome") .. " | passive: " ..
...
function Cards.displayCard(card, x, y, width, height)
    ...
    -- Card holder
    local holder = display.newImageRect(group, "assets/7battle/card_holder_battle_m.png", width + (35 * scaleFactor),
    ...
    if cardData.E_A == true then
        ...
        if success and entranceHook and entranceHook.play then
            ...
            entranceHook.play(x, y, width, height, parentGroup, function()
                print("displayCardWithEntranceAnimation: Animação de entrada concluída para " ..
...
local function displayFormationInGrid(formationData, gridGroup, gridSlots, onComplete, invertRows)
    ...
    local function fetchCharacterData(cardId, callback)
        ...
        local query = "?id=eq." .. tostring(cardId) ..
        ...
        if formationData[i] then
            if type(formationData[i]) == "table" then
                ...
                if formationData[i].E_A then
                    local cardGroup = displayCardWithEntranceAnimation(formationData[i], x, y, slotWidth, slotHeight,
                        gridGroup, function(cg)
                            gridSlots[i] = cg;
                            slotProcessed()
                        end)
                ...
                local function handleCardData(cardData)
                    if cardData then
                        ...
                        print("Slot " .. i .. ": Carta " .. (cardData.name or "Sem Nome") ..
                                  " iniciada com roundsAttacked = 1 (fetched)")
                        if cardData.E_A == true then
                            local cardGroup = displayCardWithEntranceAnimation(cardData, x, y, slotWidth, slotHeight,
                                gridGroup, function(cg)
                                    gridSlots[i] = cg
                                    slotProcessed()
                                end)
...
local function getAttackHook(cardData)
    ...
    if success and hookModule then
        print("getAttackHook: Hook '" .. hookName .. "' carregado com sucesso para a carta " ..
                  (cardData.name or "Sem Nome"))
...
local function getSpecialAttackHook(cardData)
    ...
    if success and hookModule then
        print("getSpecialAttackHook: Hook especial '" .. spValue .. "' carregado com sucesso para a carta " ..
                  (cardData.name or "Sem Nome"))
        if hookModule.attack then
            hookModule.attack = ensureCallback(hookModule.attack)
        end
...
local function checkMiss(attacker, target, targetSlot)
    ...
    print("checkMiss: targetEva = " .. targetEva .. ", attackerPrec = " .. attackerPrec .. ", missChance = " ..
              missChance)
...
local function showPlayerDefeatOverlay()
    ...
    local halo = display.newImageRect(overlayGroup, "assets/7battle/misc_halo.png", display.contentWidth,
    ...
    local defeat = display.newImageRect(overlayGroup, "assets/7battle/battle_ending_loose.png", display.contentWidth,
        display.contentHeight)
...
local function showPlayerVictoryOverlay()
    ...
    local victory = display.newImageRect(overlayGroup, "assets/7battle/battle_ending_victory.png", display.contentWidth,
        display.contentHeight)
    ...
    timer.performWithDelay(1000, function()
        local touchRect = display.newRect(overlayGroup, display.contentCenterX, display.contentCenterY,
...
local function simulateTurnActions()
    ...
    local function getCriticalMultiplierWithEffect(attacker, target, callback, playAnimation, targetSlot)
        ...
        if type(callback) ~= "function" then
            ...
            callback = function(_)
            end -- função dummy que não faz nada
        end
        ...
        print("getCriticalMultiplierWithEffect: attackerCri = " .. attackerCri .. ", targetRes = " .. targetRes ..
    ...
    battleFunctions.applyDamage = function(attacker, target, damage, targetSlot)
        ...
        if target.shieldHitsRemaining and target.shieldHitsRemaining > 0 then
            print((target.name or "Carta") .. " bloqueou o ataque com escudo! Restam " ..
                      (target.shieldHitsRemaining - 1) .. " bloqueios.")
        ...
        if target.shareVisionHitsRemaining and target.shareVisionHitsRemaining > 0 then
            ...
            if math.random() < 0.4 then
                print((target.name or "Carta") .. " bloqueou o ataque com shareVision! Restam " ..
                          target.shareVisionHitsRemaining .. " bloqueios.")
                ...
                if grp then
                    ...
                    local img = display.newImageRect(grp, "assets/7battle/rinnegan.png", -- substitua pelo seu PNG
                    w * 0.9 * deviceScaleFactor, h * 0.5 * deviceScaleFactor)
        ...
        battleFunctions.getCriticalMultiplierWithEffect(attacker, target, function(critMultiplier)
            ...
            print("Dano de " .. appliedDamage .. " aplicado de " .. (attacker.name or "Sem Nome") .. " a " ..
                      (target.name or "Sem Nome") .. " no slot " .. targetSlot .. ". HP anterior: " .. prevHP ..
    ...
    battleFunctions.applyHealing = function(attacker, target, healAmount, targetSlot)
        ...
        print(
            "Cura de " .. healAmount .. " aplicada a " .. (target.name or "sem nome") .. ". HP anterior: " .. prevHP ..
                " | HP atual: " .. target.hp .. " | Applied heal: " .. appliedHeal)
    ...
    --------------------------------------------------------------------------------
    local function checkAndActivateSpecialEffect(cardData, cardGroup, attacker, target, battleFunctions, targetSlot,
        callback)
        local ok, err = pcall(function()
            ...
            print("[checkAndActivateSpecialEffect] " .. (cardData.name or "Sem Nome") .. " | Round: " .. r ..
                      " | isSpecial: " .. tostring(isSpecial) .. " | Should Activate Effect: " ..
                ...
                if cardData.effectActive then
                    ...
                    transition.to(cardData.specialEffectSprite, {
                        ...
                        onComplete = function()
                            ...
                            print("[checkAndActivateSpecialEffect] Effect REMOVED for " .. (cardData.name or "Sem Nome"))
                        end
                    })
                else
    ...
    local function manageCardTurn(cardData, action, value)
        ...
        elseif action == "block" then
            if (cardData.shieldHitsRemaining or 0) > 0 or (cardData.shareVisionHitsRemaining or 0) > 0 or
                cardData.lastAttackMiss then
            ...
            if cardData.group and not cardData.stunImage then
                local stunImage = display.newImageRect("assets/7battle/icon_buffer_1.png", 60 * deviceScaleFactor,
        ...
        elseif action == "shield" then
            ...
            if cardData.group then
                if not cardData.defenseIcon then
                    defenseIcon = display.newImageRect("assets/7battle/icon_buffer_2.png", 60 * deviceScaleFactor,
                        60 * deviceScaleFactor)
        ...
        elseif action == "shareVision" then
            ...
            if cardData.group then
                if not cardData.shareVisionIcon then
                    shareVisionIcon = display.newImageRect("assets/7battle/icon_buffer_14.png", 65 * deviceScaleFactor,
                            ...
                            onComplete = function()
                                timer.performWithDelay(200, function()
                                    ...
                                    transition.to(cardData.group, {
                                        ...
                                        alpha = 1.0,
                                        onComplete = function()
                                            if cardData.group.healthBar and cardData.group.healthBar.numChildren >= 2 then
                ...
                showAdvanceButton(function()
                    ...
                    timer.performWithDelay(750, function()
                        if not playerGridGroup then
                            ...
                            playerGridGroup = createCardGrid(parent, display.contentCenterX, display.contentHeight -
                                (400 / 1920) * display.actualContentHeight)
                        end
                                ...
                                localOpponentModule.fetchFormationData(teamG .. "002", function(newOppFormation, err)
                                    ...
                                    for i = 1, #newOppFormation do
                                        if oldFormation[i] and newOppFormation[i] then
                                            ...
                                            newOppFormation[i].originalHP = oldFormation[i].originalHP or
                                                                                newOppFormation[i].hp
                                        end
                                    end
                                    ...
                                    displayFormationInGrid(newOppFormation, opponentGridGroup, gridSlotsOpponent,
                                        function()
                            ...
                            if pData.effectActive and pData.specialEffectSprite then
                                transition.to(pData.specialEffectSprite, {
                                    ...
                                    onComplete = function()
                                        ...
                                        pData.specialEffectSprite = nil
                                        pData.effectActive = false
                                        specialHook.attack(pData, targetData, battleFunctions, targetSlot,
                                            onAttackComplete)
                                    end
                                })
                            else
                            ...
                            if oData.effectActive and oData.specialEffectSprite then
                                transition.to(oData.specialEffectSprite, {
                                    ...
                                    onComplete = function()
                                        ...
                                        oData.specialEffectSprite = nil
                                        oData.effectActive = false
                                        specialHook.attack(oData, targetData, battleFunctions, targetSlot,
                                            onAttackComplete)
                                    end
                                })
                            else
...
local function defineFormationCenters()
    ...
    print(string.format("Centro da formação do oponente: (%.1f, %.1f)", _G.opponentFormationCenter.x,
        _G.opponentFormationCenter.y))

    _G.playerFormationCenter = computeFormationCenter(gridSlotsPlayer)
    print(string.format("Centro da formação do jogador: (%.1f, %.1f)", _G.playerFormationCenter.x,
