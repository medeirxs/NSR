-- interfaces/growing/raiseSelect.lua
...
function scene:create(event)
	...
	local topBack = topBack.new({
		title = "Selecione a carta para avançar ",
		func = "interfaces.growing.raise",
	})
	sceneGroup:insert(topBack)

	-- pega userId (pode vir de params ou do storage local)
	local data = userDataLib.load() or {}
	local localUserId = tonumber(data.id) or 0
	...
	network.request(url, "GET", function(evt)
		...
		for _, c in ipairs(chars) do
			local cell = require("components.cardCell").new({
...
function scene:hide(event)
	if event.phase == "will" then
		composer.removeScene("interfaces.growing.raiseSelect")
	end
end
